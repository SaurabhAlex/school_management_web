...
export const ManageStudents = ({ title = "Manage Students", showEmailField = true }: ManageStudentsProps) => {
  ...
  const validateField = (name: string, value: string): string => {
    switch (name) {
      ...
      case 'email':
        return showEmailField ? (/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value) ? '' : 'Please enter a valid email address') : '';
      default:
        return '';
    }
  };
  ...
  const validateForm = () => {
    ...
    Object.keys(formData).forEach(key => {
      if (key === 'email' && !showEmailField) return;
        ...
        <Table>
          <TableHead>
            <TableRow>
              ...
              {showEmailField && <TableCell sx={{ fontWeight: 500 }}>Email</TableCell>}
          ...
          <TableBody>
            {students?.map((student: Student) => (
              <TableRow key={student.id}>
                ...
                {showEmailField && <TableCell>{student.email}</TableCell>}
                <TableCell>
                  <IconButton 
